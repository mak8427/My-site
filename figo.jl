...
# ╔═╡ d7ed250c-a4e9-4d37-85fd-6a7bc1bce690
begin
	using Random
	using CUDA
	using Plots
...
# ╔═╡ 968eaa30-eb56-4b62-af0b-67e1cc32a93a
md"""
Flip a fair coin 100 times—it gives a sequence of heads (H) and tails (T). For each HH in the sequence of flips, Alice gets a point; for each HT, Bob does, so e.g. for the sequence THHHT Alice gets 2 points and Bob gets 1 point. Who is most likely to win?
    — Daniel Litt (@littmath)  [March 16, 2024](https://twitter.com/littmath/status/1769044719034647001)

![image](https://ro-che.info/img/alice-bob-flipping-coins/poll.jpeg)
"""

# ╔═╡ 4da17079-b79d-421e-871e-a030380f068b
begin
	N = 10_000_001
	flip_length = 100
end

# ╔═╡ b9d00c10-a2cf-49e6-9fbb-89ae7bb07552
md"""
true = Head 
false = Tail
"""

# ╔═╡ 1bda292b-87e2-4402-b78c-7c2ee8b93534
function flipper!(flips)
    i = 1
    alice_points = 0
    bob_points = 0
...
# ╔═╡ 1d7752ab-42d4-44a5-8059-29bb9137449b


# ╔═╡ 4208231e-6592-4305-b646-acffe9cf55a6
begin

	# Initialize shared arrays for results
	const Alice = Vector{Int}(undef, N)
	const Bob = Vector{Int}(undef, N)
	
	# Use threading to parallelize the loop
	Threads.@threads for i in 1:N
	    flips = bitrand(flip_length)
	    alice_points, bob_points = flipper!(flips)
	    Alice[i] = alice_points
	    Bob[i] = bob_points
	end
	
	# Calculate and print results
	println(sum(Alice) / length(Alice))
	println(sum(Bob) / length(Bob))
end

# ╔═╡ 63b65919-e328-4257-9501-942112397139
begin
	Alice_win = 0
	Bob_win = 0
	for i in 1:N-2
	...
	# Print results
	println("Alice wins: ", round(Alice_win/N* 100,digits=2))
	println("Bob wins: ", round(Bob_win/N* 100,digits=2))
		
end

# ╔═╡ 27dc5b5c-5039-496d-a0ea-7cac7177a10a
begin
	histogram(Alice, bins=50, alpha=0.5, label="Alice's Points", title="Distribution of Points", xlabel="Points", ylabel="Frequency", normalize=true)
	
	histogram!(Bob, bins=45, alpha=0.5, label="Bob's Points", normalize=true)
end

# ╔═╡ ec577520-8e3d-4082-8aff-d4577bab905e


# ╔═╡ d0bde76b-c60a-4ce8-bdce-146724955720
...
manifest_format = "2.0"
project_hash = "39d4ed6812a49567a9bb45197b778180a46b8e3a"
